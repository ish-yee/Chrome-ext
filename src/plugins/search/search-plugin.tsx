...
const generateSearchUrl = (provider: Provider, query: string) => {
  return {
    ...
    yt: `https://www.youtube.com/results?search_query=${encodeURIComponent(
      query
    )}`,
...
const ConfigScreen = ({
  currentConfig,
  saveConfiguration,
}: WidgetConfigurationScreenProps<WidgetConfig>) => {
  const [defaultProvider, setDefaultProvider] = useState<Provider>(
    currentConfig ? currentConfig.defaultProvider : "google"
  );
  ...
  return (
    <div className="SearchWidget-config">
      ...
      <Button
        ...
        onClick={() => saveConfiguration({ defaultProvider })}
      >
        {t("save")}
      </Button>
    </div>
    ...
    <div className="SearchWidget">
      <div className="providers">
        <FloatingDelayGroup delay={200}>
          {providers.map((p) => {
            return (
              <Tooltip key={p} label={providersPretty[p]} placement="top">
                <Button
...
const onCommandInput: OnCommandInputCallback = async (text: string) => {
  ...
  if (!query) return [];
  return providers
    .filter((p) => (shouldFilterProviders ? p === command : true))
    .map((p) => {
      const url = generateSearchUrl(p, query);

      return {
        icon: providersIcons[p],
        text: translate("search-plugin.searchProviderForQ", {
          provider: providersPretty[p],
          query,
        }),
