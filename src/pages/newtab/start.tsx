...
import {
  ...
  useWidgetWindows,
} from "@components/WidgetExpandArea";
...
import Announcements from "../newtab/Announcements"; // Adjust path as needed
...
const CommandMenu = lazy(() =>
  import("@components/command-menu/CommandMenu").then((m) => ({
    default: m.CommandMenu,
  }))
);
const BookmarksBar = lazy(() =>
  import("./components/BookmarksBar").then((m) => ({ default: m.BookmarksBar }))
);

const loadMotionFeatures = () =>
  import("@utils/motion/framer-motion-features").then((res) => res.default);

const useSidebarOrientation = () => {
  const [sidebarOrientation] = useBrowserStorageValue(
    "sidebarOrientation",
    "auto"
  );
  const [winOrientation, setWinOrientation] = useState<
    "landscape" | "portrait"
...
const Start = () => {
  const switchToFolder = (folder: Folder) => {
    if (hasDetachedWindows()) {
      ...
      alert(
        `You have detached windows, please close them before switching folders`
      );
      return;
    }
  ...
  const swithFolderUp = () => {
    switchToFolder(
      folders[
        activeFolderIndex === 0 ? folders.length - 1 : activeFolderIndex - 1
      ]
    );
  };
  ...
  const swithFolderDown = () => {
    switchToFolder(
      folders[
        activeFolderIndex === folders.length - 1 ? 0 : activeFolderIndex + 1
      ]
    );
  };
  ...
  const [rememberLastFolder] = useBrowserStorageValue(
    "rememberLastFolder",
    false
  );
  const [lastFolder, setLastFolder] = useBrowserStorageValue(
    "lastFolder",
    "home"
  );
  ...
  const animationDirection =
    previousActiveFolderIndex === undefined ||
    previousActiveFolderIndex === activeFolderIndex
      ? null
      ...
      <MotionConfig transition={{ duration: 0.2, ease: "easeInOut" }}>
        ...
        <AnimatePresence>
          <m.div
            className={clsx(
              "StartPage",
              `${sidebarOrientation}-sidebar`,
              showBookmarksBar && "with-bookmarks-bar"
            )}
            key="start-page"
          >
            {showBookmarksBar && (
              <Suspense
                fallback={<div className="bookmarks-bar-placeholder" />}
              >
                <BookmarksBar />
              </Suspense>
            )}
      ...
      if (overlapItems.length) {
        ...
        overlapItems.map((item) => {
          const columns = Math.max(
            ...layoutWithoutOverlay.map((i) => i.x + i.width),
            0
          );
          const rows = Math.max(
            ...layoutWithoutOverlay.map((i) => i.y + i.height),
            0
          );
